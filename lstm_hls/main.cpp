...
FP_TYPE* buf = nullptr;		// 图片缓存
byte* label = nullptr;		// 标签缓存

int mnist_app_set(const char* data_path, const char* label_path);
FP_TYPE mnist_app_test_all();
void mnist_app_end();

int main()
{
    int t = mnist_app_set("D:/Project_fpga_yjl/DLA_lstm/lstm_pytorch/MNIST/raw/t10k-images.idx3-ubyte",
    						"D:/Project_fpga_yjl/DLA_lstm/lstm_pytorch/MNIST/raw/t10k-labels.idx1-ubyte");
    if (t == -1) return 0;
...
int mnist_app_set(const char* data_path, const char* label_path)
{
	...
	// 分配空间
	buf = new FP_TYPE[img_row * img_col];
...
inline void normalize(FP_TYPE* out, byte* in, int size, int max_val)
{
	for (int i = 0; i < size; i++)
		out[i] = (FP_TYPE)in[i] / max_val;
}
...
FP_TYPE mnist_app_test_all()
{
	...
	delete tmp;
	return (FP_TYPE)match_num / img_num;
}
